...
# I CHANGED THIS ALOT IF ITS NOT WORKING GO GET FRESH FILE
class Texture:
    def __init__(self, file_names=None, property_dict={}):
        # Pygame object for storing pixel data;
        # can load from image or manipulate directly
        self._surface = None
        # reference of available texture from GPU
        self._texture_ref = GL.glGenTextures(len(file_names) + 1)
        ...
        #     self.load_image(file_name1)
        #     self.upload_data(1)

        for idx, file_name in enumerate(file_names):
            self.load_image(file_name)
            self.upload_data(idx) 
    ...
    def upload_data(self, number):
        ...
        GL.glBindTexture(GL.GL_TEXTURE_2D, self._texture_ref[number])
