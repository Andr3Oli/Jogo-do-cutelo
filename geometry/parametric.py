...
class ParametricGeometry(Geometry):
    def __init__(self,
        ...
        def calc_normal(p0, p1, p2):
            ...
            if np.linalg.norm(p0) == 0: pass
            if norm > 1e-6: normal_vector = orthogonal_vector / norm
            elif np.linalg.norm(p0) != 0: normal_vector = np.array(p0) / np.linalg.norm(p0)
            else: return [float('nan'), float('nan'), float('nan')]
